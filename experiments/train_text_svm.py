"""
Text-metadata classifier, SVM variant: TF-IDF over the job/container +
network + storage I/O text (experiments/preprocess.py's text_features.csv)
+ a linear SVM instead of train_text_classifier.py's logistic regression.

Linear SVMs are a classic strong baseline for small, high-dimensional
sparse text data (TF-IDF is exactly that) -- worth comparing directly
against logistic regression rather than assuming one wins. Same
stratified cross-validation setup as train_text_classifier.py, so the two
are directly comparable in experiments/evaluate.py.

    python experiments/train_text_svm.py

Writes results/text_svm_results.json.
"""
...
def main():
    ...
    parser.add_argument("--C", type=float, default=1.0, help="SVM regularization strength (smaller = more regularized)")
    ...
    # LinearSVC exposes .coef_ just like LogisticRegression, so the same
    # top-terms extraction applies.
    ...
    out_path = results_dir / "text_svm_results.json"
    with open(out_path, "w") as f:
        json.dump(results, f, indent=2)

    print(f"[train_text_svm] accuracy={overall['accuracy']:.3f} f1={overall['f1']:.3f} -> {out_path}")
